...
void RandomTest::runTest() {
  for (int i = from; i <= to; ++i) {
    std::string inputFilename =
        "random_test/input/input_" + std::to_string(i) + ".txt";
    std::string outputFilename =
        "random_test/output/output_" + std::to_string(i) + ".txt";

    MemoryManager mm(inputFilename);

    std::string outputContent = mm.toString_ListDeclarations();
...
void RandomTest::compareOutputs() {
  ...
  for (int i = from; i <= to; ++i) {
    ...
    std::string expectedFile =
        "random_test/expected/expected_" + std::to_string(i) + ".txt";
