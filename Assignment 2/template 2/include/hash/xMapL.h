...
xMap<K, V>::xMap(const xMap<K, V> &map)
{
  ...
  for (int i = 0; i < map.capacity; i++)
  {
    ...
    {
      this->table[i].add(new Entry(entry->key, entry->value));
    }
  }
}
...
V xMap<K, V>::put(K key, V value)
{
  ...
  DLinkedList<Entry *> &curEntryList = this->table[index];

  for (auto it = curEntryList.begin(); it != curEntryList.end(); it++)
  {
  ...
  Entry *insEntry = new Entry(key, value);
  curEntryList.add(insEntry);
  this->count++;
...
V xMap<K, V>::remove(K key, void (*deleteKeyInMap)(K))
{
  ...
  for (auto it = curEntryList.begin(); it != curEntryList.end(); it++)
  {
    if (this->keyEQ((*it)->key, key))
    {
      ...
      curEntryList.removeItem((*it), this->deleteEntry);
...
bool xMap<K, V>::containsValue(V value)
{
  ...
  for (int index = 0; index < this->capacity; index++)
  {
    ...
    for (auto it = curEntryList.begin(); it != curEntryList.end(); it++)
    {
      if (this->valueEQ((*it)->value, value))
      {
        return true;
      }
    }
